...
class RuleSuite extends FunSuite {
  test("merge") {
    ...
    val rule = Rule(SortAppl("Exp", List()), Some(TypeAppl("TFun", List(TypeAppl("TInt", List()), TypeVar("t15")))), List(ScopeVar("s")), State(TermAppl("Fun", List(TermVar("x55791"))), List(TypeOf(SymbolicName("Var", "x"), TypeAppl("TFun", List(TypeVar("t55788"), TypeVar("t55789")))), Res(SymbolicName("Var", "x658"),NameVar("d659")), TypeOf(NameVar("d659"),TypeAppl("TFun", List(TypeVar("t93"), TypeAppl("TFun", List(TypeVar("t19"), TypeVar("t15")))))), Recurse(TermVar("x55791"),List(),Some(TypeVar("t55789")),SortAppl("Type", List())), Recurse(TermVar("x55790"),List(),Some(TypeVar("t55788")),SortAppl("Type", List())), Recurse(TermVar("x55887"),List(),Some(TypeVar("t55885")),SortAppl("Type", List())), Recurse(TermVar("x55886"),List(),Some(TypeVar("t55884")),SortAppl("Type", List())), Recurse(TermVar("x56245"),List(),Some(TypeVar("t56243")),SortAppl("Type", List())), Recurse(TermVar("x56244"),List(),Some(TypeVar("t56242")),SortAppl("Type", List()))),List(Dec(ScopeVar("s575"),SymbolicName("Var", "x")), DirectEdge(ScopeVar("s575"),Label('P'),ScopeVar("s")), Dec(ScopeVar("s657"),SymbolicName("Var", "x576")), DirectEdge(ScopeVar("s657"),Label('P'),ScopeVar("s575")), Ref(SymbolicName("Var", "x658"),ScopeVar("s657"))),TypeEnv(),Resolution(),SubtypeRelation(),List()))
    val recurse = Recurse(TermVar("x55791"),List(),Some(TypeVar("t55789")),SortAppl("Type", List()))
    val other = Rule(SortAppl("Type", List()), Some(TypeAppl("TFun", List(TypeVar("t1'"), TypeVar("t2'")))), List(), State(TermAppl("FunType", List(TermVar("t1"), TermVar("t2"))),List(Recurse(TermVar("t2"),List(),Some(TypeVar("t2'")),SortAppl("Type", List())), Recurse(TermVar("t1"),List(),Some(TypeVar("t1'")),SortAppl("Type", List()))),List(),TypeEnv(),Resolution(),SubtypeRelation(),List()))
